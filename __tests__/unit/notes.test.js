...
const { deleteDatetoLabel, deleteLabel } = require('../../source/Notes/label-helper')
const fs = require('fs');
...
describe('app', () => {
    ...
    beforeEach(() => {
      ...
      it('should render a Markdown entry', async () => {
        // Mock the readFile implementation to return the markdown content
        readFileSpy.mockImplementation((path, options, callback) => callback(null, markdown));
    
        // Mock the DOM elements
        document.body.innerHTML = '<textarea id="markdown"></textarea><div id="markdownPreview"></div>';
    
        // Render Markdown entry
        renderMarkdownEntry(day, month, year);
    
        // Check that the file was read and the content was set in the DOM
        expect(readFileSpy).toHaveBeenCalledWith(expect.any(String), 'utf-8', expect.any(Function));
        expect(document.getElementById('markdown').value).toBe(markdown);
        expect(document.getElementById('markdownPreview').innerHTML).toBe(marked(markdown));
      });
